...
cdef class PySolution:
	...
	cpdef create_random_params(self, centers, double width):
		...
		cdef double* centers_a = arr_to_ptr(8)
		cdef double* centers_b = arr_to_ptr(8)
		cdef int k
		for k in range(8):
		...
		ret = np.zeros((2,8)).astype(np.double)
	...
	cpdef double i_fast(self, params, int intervals):
		...
		cdef double* p_a = arr_to_ptr(8)
		cdef double* p_b = arr_to_ptr(8)
		cdef int i,j
		for i in range(8):
			p_a[i] = params[0][i]
			p_b[i] = params[1][i]
	...
	cpdef optimize(self, int iterations, int num_starts, double target):
		...
		cdef np.ndarray[double, ndim = 3] params = np.zeros((k,2,8)).astype(np.double)

		cdef int a,i,j
		for a in range(k):
			#cdef np.ndarray[double, ndim = 2] temp_params
			#temp_params = np.zeros((2,8)).astype(np.double)
			for i in range(2):
				for j in range(8):
	...
	cpdef efficient_optimize(self, params, int max_iters, double threshold, double initial_width):
		...
		cdef np.ndarray[double, ndim = 2] ret_params = np.zeros((2,8)).astype(np.double)
